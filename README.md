# OneStable Arbitrage Bot

## ⚠️ WARNING: Use at Your Own Risk

The OneStable Bot requires an Ethereum private key. **Your funds are AT RISK when you run the OneStable Arbitrage Bot**. Read the _Preparation_ section below. The bot is provided as-is, with the authors and the Bprotocol Foundation not liable for any losses.
...
The OneStable is an open-source system that anyone can run, consisting of the OneStable smart contract, and the OneStable Arbitrage Bot.

The OneStable Arbitrage Bot identifies arbitrage opportunities and closes them by executing trades using the OneStable Smart Contract. The bot can search for opportunities between Carbon and Carbon forks, Bancor, and other DEXes. This enhances market efficiency by aligning Carbon, Carbon forks, and Bancor with market trends.
...
Install OneStable Arbitrage Bot from PyPi using the following command:
...
Due to the architectural differences of the Apple Silicon chip, some Python packages may not install correctly using the standard method. Follow the instructions below to create a compatible conda environment, and then install OneStable Arbitrage Bot:
...
```bash
conda create -n onestable_bot_env python=3.9
conda activate onestable_bot_env
```
...
The OneStable Arbitrage Bot needs access to an Ethereum wallet's private key. **THIS KEY IS AT RISK AND SHOULD NOT BE USED ELSEWHERE**. Maintain some ETH in the wallet for gas fees and regularly sweep profits. Supply the private key to the bot using an environment variable, as shown:
...
You can configure the OneStable Arbitrage Bot using the options in the `@click.option` section of `main.py`. An overview of options is provided below:
...
- **version_check_frequency** (int): The frequency (in bot cycles) the bot will check the version of the OneStable smart contract. Set to -1 to disable. 
