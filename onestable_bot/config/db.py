"""
OneStable bot config -- database configuration
"""
__VERSION__ = "1.0.4"
...
class _ConfigDBPostgres(ConfigDB):
    """
    OneStable bot config -- database [Postgres]
    """
...
class _ConfigDBSqlite(ConfigDB):
    """
    OneStable bot config -- database [Sqlite]
    """
...
class _ConfigDBMemory(ConfigDB):
    """
    OneStable bot config -- database [Memory]
    """
...
class _ConfigDBSdk(ConfigDB):
    """
    OneStable bot config -- database [SDK]
    """
...
class _ConfigDBUnitTest(ConfigDB):
    """
    OneStable bot config -- database [UnitTest]
    """
    DATABASE = S.DATABASE_UNITTEST
    POSTGRES_USER = os.environ.get("POSTGRES_USER")
    POSTGRES_PASSWORD = os.environ.get("POSTGRES_PASSWORD")
